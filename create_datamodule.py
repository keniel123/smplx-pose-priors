...
class HandPriorDatasetCreator:
    ...
    def calculate_sampling_rates(self, dataset_info: Dict[str, int]) -> Dict[str, Dict[str, float]]:
    ...
    def save_splits(self, splits: Dict[str, List[Tuple[str, int]]],
    ...
    def create_dataset(self, seed: int = 42) -> None:
        ...
        # Print summary
        print("\\n📈 Sampling Summary:")
        total_available = sum(info['total_available'] for info in sampling_rates.values())
        total_target = sum(info['target_samples'] for info in sampling_rates.values())

        for priority in ['high', 'medium', 'low']:
            info = sampling_rates[priority]
            print(f"  {priority.upper()}: {info['target_samples']:,} samples "
